...
class ProgressService:
    ...
    def snapshot(self, project_id: str) -> dict[str, Any]:
        ...
        with connect(self.settings) as db:
            ...
            assessment = db.execute("SELECT pa.assessment_id,pa.goal_revision_id,pa.repository_revision,pa.progress_percent,pa.confidence,pa.freshness_state,pa.summary,pa.item_results,pa.evidence_refs,pa.created_at FROM prime_core.progress_assessments pa WHERE pa.project_id=%s AND pa.freshness_state='CURRENT' AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(pa.evidence_refs,'[]'::jsonb)) AS ref(value) WHERE NOT EXISTS (SELECT 1 FROM prime_core.source_references sr WHERE sr.project_id=pa.project_id AND sr.source_reference_id=ref.value AND sr.freshness_state='CURRENT')) ORDER BY pa.created_at DESC LIMIT 1", (project_id,)).fetchone()
