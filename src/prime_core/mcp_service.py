...
class MCPService:
    ...
    def call(self, token: str, tool: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool == "prime_memory_store":
            ...
            result = self.memory.store(project_id, body["content"], content_class, (body.get("working_context") or {}).get("git_commit"))
            return {"status": result.get("status", "degraded").lower(), "memory_id": result.get("memory_id"), "hindsight_document_id": result.get("memory_id"), "durability_verified": result.get("status") == "STORED", "provenance_refs": []}
    ...
    def _timeline(self, project_id: str, limit: int) -> dict[str, Any]:
        with connect(self.settings) as db:
            rows = db.execute("SELECT memory_id,content_class,status,created_at,source_revision FROM prime_core.memory_records WHERE project_id=%s ORDER BY created_at DESC LIMIT %s", (project_id, limit)).fetchall()
            return {"project_id": project_id, "results": [dict(row) for row in rows]}

    def _get(self, project_id: str, memory_id: str) -> dict[str, Any]:
        with connect(self.settings) as db:
            row = db.execute("SELECT memory_id,project_id,content_class,status,content,source_revision,branch_context,created_at,metadata FROM prime_core.memory_records WHERE project_id=%s AND memory_id=%s", (project_id, memory_id)).fetchone()
