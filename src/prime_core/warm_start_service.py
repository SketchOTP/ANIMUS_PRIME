...
class WarmStartService:
    ...
    def run(self, project_id: str, authority_paths: list[str], notion_source_binding_ids: list[str], confirm: bool = False) -> dict[str, Any]:
        ...
        status = "CURRENT" if not rejected and not any(item.get("status") in {"DEGRADED", "UNAVAILABLE"} for item in skipped) and not any(item.get("status") in {"DEGRADED", "UNAVAILABLE"} for item in admitted) else "PARTIAL"
