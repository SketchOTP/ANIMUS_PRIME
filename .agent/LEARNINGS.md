...
## L-PRIME-DESTRUCTIVE-SAGA-RECOVERY-092

- Date: 2026-08-18
- Learning ID: L-PRIME-DESTRUCTIVE-SAGA-RECOVERY-092
- Fact or lesson: Destructive workflow recovery cannot infer completion from a missing local project row alone. A purge plan and stable workflow identity must survive long enough to reconcile Hindsight, repository, and local-resource deletion; only after those dispositions are recorded may the system scrub the project and retain a minimal tombstone. Every resumed destructive action must also require a fresh authorized preflight, while preserving the same active workflow identity.
- Evidence location: `evidence/phase15/qualification-continuation-092.md`; `evidence/phase15/qualification-continuation-092-interruption-matrix.md`; `src/prime_core/lifecycle_service.py`; `src/prime_node/service.py`; `migrations/prime/0040_destructive_lifecycle_sagas.sql`.
- Confidence: VERIFIED
- Scope: Continuation 092 durable DELETE/quarantine/PURGE and DOD-077 operator qualification.
- Supersedes learning: none

## L-PRIME-FORK-IDENTITY-AND-BROWSER-TOOLING-093

- Date: 2026-08-18
- Learning ID: L-PRIME-FORK-IDENTITY-AND-BROWSER-TOOLING-093
- Fact or lesson: A valid PRIME Fork is an independently durable product identity, not a filesystem copy: repository/revision, mutable authority, Goal/baseline, MCP grant, Notion record, Hindsight bank, Brain, memory, and activity must all be child-scoped and replay-idempotent. Source cleanliness must inspect tracked state while preserving unrelated untracked operator files. On quota-constrained Atlas, gstack Playwright can remain persistent and non-privileged by placing browsers and temporary extraction under `/mnt/storage1tb`; Chromium's supported no-sandbox path is required when the host denies the unprivileged sandbox.
- Evidence location: `evidence/phase15/qualification-continuation-093.md`; `evidence/phase15/qualification-continuation-093-fork-isolation-matrix.md`; `evidence/phase15/qualification-continuation-093-polish-matrix.md`.
- Confidence: VERIFIED
...
- Fact or lesson: PRIME's post-V1 UX gap is structural rather than merely cosmetic. The qualified shell mounts most global, setup, project, integration, lifecycle, and diagnostic surfaces in one long document with nearly equal visual weight. The approved external design reference demonstrates the useful corrective pattern: compact project identity, contextual tabs, one active surface, concise status summaries, progressive disclosure, accessible icon actions, and restrained neon emphasis. Transferring that language must preserve PRIME's richer provenance, authority, degraded-state, security-negative, and destructive-action contracts.
- Evidence location: `docs/post-v1/ANIMUS-PRIME-UX-DESIGN-REMEDIATION-BRIEF.md`; PRIME `apps/web/index.html` at `a4ef39e`; external reference source at `/home/sketch/Projects/animus_directive` baseline `550b303`.
- Confidence: VERIFIED
